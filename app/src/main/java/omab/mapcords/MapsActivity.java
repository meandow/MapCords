...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback, SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void setPin(String name, double firstValue, double secondValue) {
        ...
        if (mapReady) {
            switch (PrefHelp.getCoordinateSystemPreference(sharedPreferences)) {
                case Constants.NavigationMode.NAVIGATION_MODE_SWEREF99:
                    position = PositionHelper.getWgsFromSweRef(firstValue, secondValue);
                    break;
                case Constants.NavigationMode.NAVIGATION_MODE_WGS84:
    ...
    private void updateMapType() {
        mMap.setMapType(PrefHelp.getMapSystemPreference(sharedPreferences));
    }
